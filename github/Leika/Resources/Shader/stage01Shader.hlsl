...
P2F main(V2P pixel)
{
    ...
    result.fragment = CombineRGBWithAlpha(pixel.diffuse.rgb, pixel.diffuse.a);
    
	float light = dot(pixel.worldNorm, LightDirection[0]);

	light = light * 0.5f + 0.5f;
	light = light * light;

	result.fragment = float4(light.xxx, 1);
	
	// 色指定
	//result.fragment = result.fragment * float4(1, 0, 0, 1);

	//// 色指定
	//float3 c = float3(0, 1, 0) * light;
	//result.fragment = float4(c, 1);

	// 色指定
	//result.fragment = float4(float3(0, 0, 1) * light, 1);
